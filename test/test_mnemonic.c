...
static const char TAG[] = "test_mnemonic";
...
TEST_CASE("BIP39/44 Mnemonic To Master Seed", "[bip_mnemonic]"){
    /* Typical Benchmark (uS)
     * 12-Word
     * 857,400 - default (sha engine disabled, use mbed implementations)
     * 1,273,561 - enabling sha engine in mbedtls
     * 1,273,548 - not using mbed implementation
     *
     * 24-Word
     * 1,247,750 - default
     * 1,863,712 - enabling sha engine in mbedtls
     * 1,863,728 - not using mbed implementation
     */
    ...
    ESP_LOGI(TAG, "12-Word Mnemonic Master Seed Derived in %llu uS.", t_end-t_start);
    ...
    res = bm_mnemonic_to_master_seed(guess_master_seed_bin, 
            ...
            "TREZOR");
    t_end = esp_timer_get_time();
    ESP_LOGI(TAG, "24-Word Mnemonic Master Seed Derived in %llu uS.", t_end-t_start);
    sodium_bin2hex(guess_master_seed_hex, sizeof(guess_master_seed_hex),
            guess_master_seed_bin, sizeof(guess_master_seed_bin));
    sodium_memzero(guess_master_seed_bin, sizeof(guess_master_seed_bin));
    TEST_ASSERT_EQUAL_INT_MESSAGE(E_SUCCESS, res,
...
TEST_CASE("Mnemonic to Nano (165) Private Key", "[bip_mnemonic]"){
    ...
    t_end = esp_timer_get_time();
    ESP_LOGI(TAG, "Master Seed to Private Key in %llu uS.", t_end-t_start);
